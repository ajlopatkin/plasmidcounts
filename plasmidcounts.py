...
def plasmidcounts(base_dir, file_dirs, read_type, output_dir, db_path,
                  tmp_path=None,
...
def get_parser():
    ...
    parser.add_argument("--file_dir", action="append", required=True, help="The directories where files are stored; can be specified multiple times.")
    parser.add_argument("--read_type", action="append",required=True, help="The read type for each directory in file_dirs; specify once for each file_dir.")
    ...
    parser.add_argument("--tmp_path", action="store", default=None, help="the path to store temporary files; defaults to <output_dir>/tmp_dir/")
    parser.add_argument("--contig_len_cutoff", action="store", default=500, help="the minimum length of contigs to keep")
    parser.add_argument("--mash_cutoff", action="store", default=0.05, help="mash distance to consider two contigs duplicate")
    parser.add_argument("--plasfinder_cov", action="store", default=80, help="coverage threshold for plasmidfinder results")
    parser.add_argument("--plasfinder_pid", action="store", default=90, help="pct ID threshold for plasmidfinder results")
    parser.add_argument("--read_threshold", action="store", default=5, help="threshold for number of reads in a contig (must be contained in fasta header)")
    parser.add_argument("--cover_threshold", action="store", default=1, help="threshold for coverage of a contig")
    parser.add_argument("--blast_id_cutoff", action="store", default=90, help="cutoff for plasmid identity in PLSdb blast")
    parser.add_argument("--blast_len_thresh", action="store", default=27000, help="threshold for contig length to blast")
    parser.add_argument("--overlap_cutoff_short", action="store", default=0.01, help="cutoff for overlap of blast results for short contigs")
    parser.add_argument("--overlap_cutoff_long", action="store", default=0.15, help=" cutoff for overlap of blast results for long contigs")
    parser.add_argument("--blast_len_cutoff", action="store", default=1000, help="cutoff for plasmid length in PLSdb blast")
    ...
    parser.add_argument("--num_contig_thresh", action="store", default=3, help="only filter high-variance strains with at least this many contigs")
...
def main():
    ...
    plasmidcounts(args.base_dir, args.file_dir, args.read_type, args.output_dir,
                  args.db_path, args.tmp_path, args.contig_len_cutoff, args.mash_cutoff,
                  args.plasfinder_cov, args.plasfinder_pid, args.read_threshold, args.cover_threshold,
                  args.blast_id_cutoff, args.blast_len_thresh, args.overlap_cutoff_short, args.overlap_cutoff_long,
                  args.blast_len_cutoff, args.filter_col, args.filter_std, args.num_contig_thresh)
